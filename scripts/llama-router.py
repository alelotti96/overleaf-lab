...
Config via environment variables:
    ...
    SAMPLING_DEFAULTS    optional JSON object of sampling parameters injected into
                         completion requests KEY BY KEY, each only when the request
                         does not set it, e.g. {"top_k": 20} to give every request
                         the top_k the Qwen3 model card prescribes for non-thinking
                         mode (llama-server defaults to 40, and the calling module
                         cannot send top_k itself: it is not an OpenAI parameter and
                         would break a bring-your-own-key backend). Values the client
                         DOES send always win. Empty or invalid = no injection.
...
# Optional sampling defaults (e.g. {"top_k": 20}), injected key by key into
# completion requests that do not set that key themselves. Same reasoning as
# above: only the local llama-server is behind the router, so OpenAI-incompatible
# parameters are safe to add HERE and nowhere else.
_SAMPLING_DEFAULTS = None
_sd_raw = os.environ.get("SAMPLING_DEFAULTS", "").strip()
if _sd_raw:
    try:
        _SAMPLING_DEFAULTS = json.loads(_sd_raw)
        if not isinstance(_SAMPLING_DEFAULTS, dict):
            raise ValueError("must be a JSON object")
    except Exception as _exc:
        print(f"[router] ignoring invalid SAMPLING_DEFAULTS ({_exc})", flush=True)
        _SAMPLING_DEFAULTS = None
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if (
            ...
            _mutated = True
        # Sampling defaults are per key, and the client's own value always wins.
        if _SAMPLING_DEFAULTS is not None and isinstance(payload, dict) and "completions" in suffix:
            for _key, _value in _SAMPLING_DEFAULTS.items():
