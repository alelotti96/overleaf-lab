...
function LLMSourceEditorComponent() {
    ...
    //
    // The review flag rides along for the same reason: both rail entries decide
    // whether to exist in a `hide` callback that runs during render and cannot
    // await the flags fetch.
    const features = useLLMFeatures()
    useEffect(() => {
        ;(window as any).__llmChatEnabled = features.chatEnabled
    }, [features.chatEnabled])
    useEffect(() => {
        ;(window as any).__llmReviewEnabled = features.reviewEnabled
    }, [features.reviewEnabled])
