...
Text ABOUT a rule does not break that rule: a passage that states or quotes a writing convention (an acronym list explaining that acronyms are spelled out at first use, a style note naming the phrases to avoid) is the document describing the rule, not violating it.
...
- Never assert an exact count you did not build by enumerating the cases: cite the occurrences you actually found, and when you cannot be sure the list is complete write "at least N", never an invented total. (The counts in a SCAN HINT fact are exhaustive and may be restated as such.) The same discipline applies to "never" and "nowhere": such a claim covers only the text you were given, so when that text is one chapter, say the symbol is not defined in that chapter, not that it is never defined.
