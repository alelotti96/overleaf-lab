...
const CANDIDATES_SYSTEM_PROMPT = `You are checking ONE requirement of a writing guideline against numbered candidate passages extracted from a LaTeX document. The passages were selected mechanically; many will be perfectly fine. For EACH candidate, judge only whether the passage VIOLATES the requirement, using the passage and the context around it: support that satisfies the requirement (a \\cite, concrete numbers, a definition) often sits in the sentence before or after the highlighted expression, and if it is there the candidate does not violate. A passage that states or quotes the rule itself (an acronym list explaining the convention, a note naming the phrases to avoid) does not violate the rule it describes. Judge what is in front of you, completely: "cannot verify" is not an answer. Write "reason" in the same language as the requirement, one short sentence. Return ONLY a JSON object shaped {"items": [{"index": 1, "reason": "...", "violates": "yes"}]}, one item per candidate, in order.`
