...
// overleaf-lab: completion models regularly echo what is already on screen: the
// suggestion arrives as "the sentence so far + the continuation", and the editor
// then shows the sentence twice (observed live: "Sono un genio incompreso. Eppure"
// suggested onto a paragraph already ending with those words). No prompt makes a
// small model reliable at not doing this, so the echo is removed mechanically:
// the longest tail of the text before the cursor that the suggestion starts with
// is cut from its head, and the longest head of the text after the cursor that
// the suggestion ends with is cut from its tail. Cutting the head overlap is also
// what completes a half-typed word correctly ("...la" + "lavagna" -> "vagna").
// A suggestion that was ALL echo becomes empty, and the editor shows nothing.
function trimEchoedOverlap(suggestion, leftContext, rightContext) {
    let out = String(suggestion || '')
    const left = String(leftContext || '')
    const right = String(rightContext || '')
    for (let len = Math.min(out.length, left.length, 400); len > 0; len--) {
        if (out.startsWith(left.slice(left.length - len))) {
            out = out.slice(len)
            break
        }
    }
    // The right side needs a floor: a 1-3 character "overlap" with what follows
    // (a period, a space) is punctuation, not an echo.
    for (let len = Math.min(out.length, right.length, 400); len >= 4; len--) {
        if (out.endsWith(right.slice(0, len))) {
...
// overleaf-lab: small completion models fall into degenerate loops on prose: the
// suggestion repeats one sentence forever, usually a sentence that is already on
// screen (observed live with a 1.5B coder model on Italian text). No sampling
// setting removes this reliably, so it is cut mechanically: walk the suggestion
// sentence by sentence and stop at the first one that, normalised, already
// appeared earlier in the suggestion itself or anywhere in the tail of the text
// before the cursor. Short fragments are exempt: "No." twice is legitimate prose.
function cutDegenerateRepetition(suggestion, leftContext) {
    const out = String(suggestion || '')
    const norm = s => s.toLowerCase().replace(/\s+/g, ' ').trim()
    const leftTail = norm(String(leftContext || '').slice(-600))
    const seen = new Set()
    const unitRe = /[^.!?\n]*(?:[.!?\n]+|$)/g
    let m
    while ((m = unitRe.exec(out))) {
        if (m[0].length === 0) break
        // Trailing punctuation is dropped from the comparison: the same sentence
        // once with "." and once with "!" is still the same loop.
        const n = norm(m[0]).replace(/[.!?]+$/, '')
        if (n.length >= 12) {
            if (seen.has(n) || leftTail.includes(n)) {
                return out.slice(0, m.index).replace(/\s+$/, '')
            }
            seen.add(n)
        }
    }
...
async function completion(req, res) {
    ...
    // overleaf-lab: personal LLM settings are honoured ONLY while the instance
    // allows them, exactly as the chat and the model list already do
    // (`Settings.llm.allowUserSettings`, set from LLM_ALLOW_USER_SETTINGS).
    //
    // The two completion paths below used to skip this check, so a `llmApiUrl` /
    // `llmApiKey` written on a user document while bring-your-own-key was still
    // open kept sending this instance's completion traffic to that endpoint after
    // the feature was switched off - and the flag is precisely what makes the
    // fetch target un-attacker-controlled, since it is what gates the routes that
    // write those fields. Reading them behind the flag makes the decision the same
    // one everywhere: no flag, no personal endpoint.
    const allowPersonalSettings = !!(Settings.llm && Settings.llm.allowUserSettings)
    ...
    if (userId && allowPersonalSettings) {
        try {
            const user = await User.findById(
                userId,
    ...
    if (
        !usingPersonalCompletion &&
        (!llmApiUrl || sharedCompletionDisabled) &&
        userId &&
        allowPersonalSettings // overleaf-lab: same gate as the chat path, see above
    ) {
        try {
            const user = await User.findById(
                userId,
    ...
    try {
        // overleaf-lab: resolved per request so a super-admin override takes effect
        // without a restart; falls back to the shipped default when unset.
        const { completionSystemPrompt: systemPrompt } = await getLLMPrompts()
...
${leftContext}[CURSOR]${rightContext}`
        ...
        // Strip any think tags, cut whatever the model echoed of the text
        // already around the cursor, then cut any degenerate repetition loop
        // (see the two helpers above).
        const cleaned = cutDegenerateRepetition(
            trimEchoedOverlap(
                stripThinkTags(completionText),
                leftContext,
                rightContext
            ),
            leftContext
